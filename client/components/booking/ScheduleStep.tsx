...
import { Slot, getAvailableDates, getAvailableSlots } from "@/lib/booking.api";
...
function formatMonthToISO(date: Date): string {
  const y = date.getFullYear();
  const mo = String(date.getMonth() + 1).padStart(2, "0");
  return `${y}-${mo}`;
}
...
function ScheduleStep({
  ...
  // Fetch available dates when specialist or month changes
  const fetchDates = useCallback(async () => {
    if (!specialistId) return;
    setIsLoadingDates(true);
    setDatesError(null);
    try {
      const monthStr = formatMonthToISO(viewDate);
      const result = await getAvailableDates(specialistId, monthStr);
      setAvailableDatesSet(new Set(result.availableDates || []));
    } catch {
      setDatesError("Unable to load availability. Please try again.");
  ...
  const getFirstDayOfMonth = (year: number, month: number) => {
    ...
    return day === 0 ? 6 : day - 1; // Monday-start calendar
  };
  ...
  const hasAnyAvailableDates = availableDatesSet.size > 0;
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          ...
          {isLoadingDates ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <CalendarX size={32} className="text-red-400 mb-3" />
              <p className="text-sm text-red-600 font-medium mb-3">
                {datesError}
              </p>
              <button
                onClick={fetchDates}
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white text-sm font-bold hover:opacity-90 transition-all"
              >
                <RefreshCw size={14} />
                Retry
              </button>
            ...
            <>
              <div className="grid grid-cols-7 gap-1 mb-2">
                {DAYS_SHORT.map((d, i) => (
                  <div
                    key={i}
                    className="text-center text-xs font-bold text-slate-400 h-8 flex items-center justify-center"
                  >
                    {d}
                  </div>
                ))}
              ...
              {!hasAnyAvailableDates && !isLoadingDates && (
                <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                  <p className="text-xs text-amber-700 font-medium text-center">
                    No available appointments this month. Please check the next
                    month.
                  </p>
                </div>
              )}
          ...
          {slotsError && (
            <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
              ...
              <p className="text-xs text-slate-500 mt-2">
                Please try selecting a different date or specialist
              </p>
            </div>
          )}
          ...
          {!isLoadingSlots &&
            ...
            !slotsError && (
              <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
                ...
                <p className="text-xs text-slate-500 mt-2">
                  Please try another day or check with a different specialist
                </p>
              </div>
            )}
          ...
          {selectedSlotId && (
            <div className="mt-5 pt-5 border-t border-slate-100">
              <p className="text-xs text-slate-500 mb-4">
                <span className="font-semibold">Expected duration:</span> 60
                minutes
              </p>
              <button
                onClick={onProceed}
