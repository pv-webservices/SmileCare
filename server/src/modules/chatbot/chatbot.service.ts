        ...
        const treatmentList = treatments
            .map(t => {
                const price = t.price
                    ...
                    : (t.priceRange ? t.priceRange.replace(/\$/g, 'Rs ') : 'Price on consultation');
...
`.trim();
        ...
        console.warn('[Chatbot] Clinic context fallback:', err?.message || err);
        return `
CLINIC INFO:
- Name: SmileCare Premium Dental Clinic
- Hours: ${process.env.CLINIC_HOURS || 'Monday-Saturday, 9 AM - 6 PM IST'}
...
- Cancellation: Minimum 24 hours notice required
- Payment: UPI, Cards, Net Banking, Wallets accepted
- Treatment catalog and specialist details are temporarily unavailable from the database, so do not invent them.
`.trim();
    }
}
...
async function getPatientContext(patientId: string): Promise<string> {
    try {
        ...
        console.warn('[Chatbot] Patient context fallback:', err?.message || err);
        return 'PATIENT BOOKINGS: Booking context is temporarily unavailable.';
    }
}
